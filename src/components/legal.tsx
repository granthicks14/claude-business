...
export function LegalPage({
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="mt-3 text-[15px] sm:text-base text-muted leading-relaxed">{lead}</p>
      ...
      <div
        className="mt-8 space-y-4 leading-relaxed
          ...
          [&_li]:text-[15px] [&_li]:leading-relaxed [&_li]:pl-5 [&_li]:relative
...
export function Fact({ fact }: { fact: DataFact }) {
  return (
    <div className="rounded-xl border border-border bg-surface p-4">
      <p className="font-medium text-[15px]">{fact.what}</p>
      <p className="text-[15px] text-muted mt-1.5 leading-relaxed">{fact.detail}</p>
    </div>
  );
}
