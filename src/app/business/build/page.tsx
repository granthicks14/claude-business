          ...
          return (
            <li key={s.kind} className="animate-stagger h-full" style={{ ["--d"]: `${i * 45}ms` } as React.CSSProperties}>
              ...
              >
                ...
                <span className="block text-xs text-muted leading-relaxed mt-1">{s.produces}</span>
              </button>
            </li>
          );
        })}
          ...
          {missing.length === 0 ? (
            ...
            <Card className="p-4">
              ...
              <ul className="mt-2.5 space-y-1.5">
                {missing.map((m) => (
                  <li key={String(m.field)} className="text-xs flex items-center gap-2">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <a href="#prompt-preview" className="text-xs text-accent-text hover:underline min-h-11 inline-flex items-center px-1">
            ...
            {toolsFor(spec.kind).map((tool) => (
              <li key={tool.name}>
                <Card className="p-3.5 h-full flex flex-col">
                  ...
                  <p className="text-xs text-muted leading-relaxed mt-1 flex-1">{tool.what}</p>
                  <p className="text-xs text-faint leading-relaxed mt-1.5">{tool.free}</p>
                  <a
                    href={tool.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-2.5 inline-flex items-center gap-1 text-xs text-accent-text hover:underline min-h-8"
                  >
                    Open {tool.name}
